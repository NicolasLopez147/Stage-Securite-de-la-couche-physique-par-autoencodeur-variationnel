...
def train_run():
    ...
    for epoch in range(1, max_epochs+1):
        ...
        pnr_E          = torch.full((batch_sz,1), 10**(SNR_BdB/10), device=device)
        decoder_in   = torch.cat([y_E_flat, H_E_hat_flat, pnr_E], dim=1)
        x_hat_E        = dec(decoder_in)
...
#Aqui el decoder recibe la entrada de y_B, H_B_hat y pnr_B, y devuelve x_hat_B.