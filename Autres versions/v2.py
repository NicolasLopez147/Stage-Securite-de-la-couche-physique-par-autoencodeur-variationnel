...
enc, dec_B = Encoder().to(device), Decoder(2*N_B + 2*N_B*N_A).to(device)

# ─── optimizadores y pérdidas ────────────────────────────────────────────────
opt_enc  = torch.optim.AdamW(enc.parameters(), lr=lr , weight_decay=1e-5) # Optimizador para el codificador (ajustar los pesos) usando Adaptive Moment Estimation
opt_decB = torch.optim.AdamW(dec_B.parameters(), lr=lr, weight_decay=1e-5) # Optimizador para el decodificador de Bob (ajusar los pesos) usando Adaptive Moment Estimation
mse = nn.MSELoss()


scheduler_enc = OneCycleLR(opt_enc, max_lr=lr, total_steps=epochs, pct_start=0.05, anneal_strategy='cos')
scheduler_decB = OneCycleLR(opt_decB, max_lr=lr, total_steps=epochs, pct_start=0.05, anneal_strategy='cos')
...
for epoch in range(1, epochs+1):
    ...
    opt_enc.step()
    scheduler_enc.step()      # Actualiza los pesos
    opt_decB.step()
    scheduler_decB.step()
